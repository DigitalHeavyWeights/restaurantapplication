...
import { KitchenTicket } from './KitchenTicket'; // Import the individual ticket component
...
export const TicketQueue: React.FC<TicketQueueProps> = ({
  ...
  // Ensure kitchenQueue is an array and filter out any invalid orders
  const validOrders = (kitchenQueue || []).filter(order => 
    order && 
    typeof order === 'object' && 
    order.orderStatus && 
    order.orderId
  ...
  const filteredOrders = statusFilter === 'all' 
    ? validOrders 
    : validOrders.filter(order => order.orderStatus.toLowerCase() === statusFilter.toLowerCase());

  const getOrderCounts = () => {
    const counts = validOrders.reduce((acc, order) => {
  ...
  if (isLoading && validOrders.length === 0) {
    return (
